...
pred.out <- tuk.out(model.obj = dat.asr, pred.obj = dat.pred, pred = "trt", sig = 0.95)
pred.out
...
dat.reduced <- asreml(Yield ~ Treatment, rcov = ~ id(Column):ar1(Row), data = dat)
reml.lrt.asreml(full.asreml.obj = dat.current, reduced.asreml.obj = dat.reduced)
...
dat.reduced <- asreml(Yield ~ Control + Herbicide + Herbicide:Rate, rcov = ~ id(Column):ar1(Row), data = dat)
reml.lrt.asreml(full.asreml.obj = dat.current, reduced.asreml.obj = dat.reduced)
